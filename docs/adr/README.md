...
| 37 | [Correction detection learns the dictionary](0037-correction-detection-learns-the-dictionary.md) | Accepted | A retype seconds after a paste is ground truth, so two bounded fail-closed AX probes diff the field against what was inserted and offer the pair with one idle-only, click-optional pill prompt (amending 0014 — approval still gates every entry); the Whisper prompt caps at 40 glossary words so learned vocabulary never widens the hallucination filter's kill radius unchecked. |
| 38 | [Speaker-attributed notes with inferred names](0038-speaker-attributed-notes-with-inferred-names.md) | Accepted | ADR 31's promised v2: a per-meeting roster feeds a second LLM-facing transcript builder (the hash text never moves), one evidence-gated Gemma call infers names people actually said (user renames always win), the owner grammar becomes the roster, and notes.md becomes a render of a structured notes.json — so a rename rewrites every mention in milliseconds with no model call. |
