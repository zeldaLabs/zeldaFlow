...
- **Humans at the approval gates, not in the loop.** Dictation is friction-free; send-email/send-message and every agent task require a Fn-tap confirmation. Learned dictionary words require explicit approval — frequency suggestions live in the Hub only, and a detected correction (the user retyping a dictated word, ADR 0037) may additionally show one idle-only, click-optional pill prompt that auto-fades into the Hub. Nothing enters the dictionary without a deliberate act.
